...
public class LevelArea : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
	...
	public void OnPointerEnter(PointerEventData eventData)
	{
		...
		AudioControler.Instance.PlaySound(AudioControler.Instance.enterLevelUiArea);
	}
	...
	public void SetAnimator(Bird bird, bool isFinal)
	{
		...
		{
			if(emotion == Var.Em.Cautious && bird.data.confidence <= -emotionRequirement +2)
				exitement = 2;
			if (emotion == Var.Em.Solitary && bird.data.friendliness <= -emotionRequirement + 2)
				exitement = 2;
