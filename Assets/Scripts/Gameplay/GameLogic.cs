...
//		Debug.Log ("extraFriendly:" + extraFriendly);
	...
	public float GetBonus(Bird playerBird, Bird enemyBird)
	{
		...
		{
			if (playerBird.emotion == enemyBird.emotion || (enemyBird.emotion == Var.Em.Neutral && playerBird.emotion != Var.Em.Neutral))
				winBonus += 4;
			if (Helpers.Instance.GetOppositeEmotion(playerBird.emotion) == enemyBird.emotion || (enemyBird.emotion != Var.Em.Neutral && playerBird.emotion == Var.Em.Neutral))
				winBonus -= 4;
	...
	public int Fight(Bird playerBird, Bird enemyBird)
	{
		...
		if (winBonus>Random.Range(0f,1f))
		{
			...
			playerBird.data.consecutiveFightsWon++;
		...
		{           
			...
			playerBird.data.consecutiveFightsWon = 0;
