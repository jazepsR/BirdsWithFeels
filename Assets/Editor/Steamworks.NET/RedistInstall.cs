...
public class RedistInstall {
	...
	static void CopyFile(string path, string filename, bool bCheckDifference) {
		...
		string strSource = System.IO.Path.Combine(System.IO.Path.Combine(strCWD, path), filename);
		string strDest = System.IO.Path.Combine(strCWD, filename);

		if (!File.Exists(strSource)) {
			Debug.LogWarning(string.Format("[Steamworks.NET] Could not copy {0} into the project root. {0} could not be found in '{1}'. Place {0} from the Steamworks SDK in the project root manually.", filename, System.IO.Path.Combine(strCWD, path)));
			return;
		}
		...
		if (File.Exists(strDest)) {
			...
			Debug.LogWarning(string.Format("[Steamworks.NET] Could not copy {0} into the project root. File.Copy() Failed. Please copy {0} into the project root manually.", System.IO.Path.Combine(path, filename)));
		}
	}
	...
	static void SetPlatformSettings() {
		foreach(var plugin in PluginImporter.GetAllImporters()) {
			...
			if(System.IO.Path.IsPathRooted(plugin.assetPath)) {
				continue;
			}

			bool didUpdate = false;
			string filename = System.IO.Path.GetFileName(plugin.assetPath);
